...
const char strPlaylistHeader[] = "#~Audio Player\r\n";
...
void InsertPlaylistElement(WCHAR* fname, UINT pos) {
	...
	if (!UseTags)
	{
		...
		if (res == FALSE)
			res = ReadApeTags(fname, &info);
		if (res == FALSE) 
		{ 
			res = ReadTagsV1(fname, &info); 
		}
	}


	if (res == FALSE && !UseTags) {//if there's no data
		item.iItem = pos;
...
void GetCurrTrackShortName(TCHAR* str) {
	...
	//***
	if (buf[0] == '>')p = buf + 1;//delete symbol of current track
	else p = buf;
...
void PlayTrackByNumber(int n) {
	...
	if (res != FALSE) { fOpenedFileTags = true; }
	else { fOpenedFileTags = false; }
...
void PlayNextTrack() {
	...
	case NORMAL:
	case REPEAT_LIST:
		InterlockedIncrement((volatile LONG*)&CurrentTrack); 
		break;
	case REPEAT_FILE: 
		PlayTrackByNumber(CurrentTrack); 
		return;
	case RANDOM:
		CurrentTrack = rand() % CountTracks; 
		break;
	}
...
void Playlist_AddDirectory(TCHAR* dir) {
	...
	while (1) {
		...
		if (FindNextFile(hSearch, &fd) == FALSE)
			break;
...
void WritePlaylistTags(HANDLE hFile, int index) {
	...
	if (str[0] == '>')p = str + 1;//delete the current track symbol
	else p = str;
